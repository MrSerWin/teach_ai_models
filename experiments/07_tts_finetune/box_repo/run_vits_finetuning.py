...
class DataCollatorTTSWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # Return a plain dict (not BatchEncoding): accelerate's send_to_device
        # skips a None speaker_id, whereas BatchEncoding.to() crashes on it.
        return dict(batch)
...
def main():
    ...
    for epoch in range(first_epoch, training_args.num_train_epochs):
        ...
        # --- ADDED: non-destructive snapshot every N epochs (for A/B). ---
        # Deep-copy the generator, strip weight-norm on the copy, save an
        # inference-ready checkpoint; the live model keeps training. The final
        # epoch is skipped here (saved to output_dir by the block below).
        if accelerator.is_main_process:
            snap_every = int(os.environ.get("SNAPSHOT_EVERY_EPOCHS", "5"))
            if (epoch + 1) % snap_every == 0 and (epoch + 1) != training_args.num_train_epochs:
                # The live model can't be deep-copied (weight-norm + autograd
                # graph). Clone its state_dict to CPU, load into a fresh
                # inference VitsModel, strip weight-norm, and save.
                m = accelerator.unwrap_model(model)
                sd = {k: v.detach().cpu().clone() for k, v in m.state_dict().items()}
                # Rebuild an inference model: same class, apply weight-norm so the
                # state_dict keys line up, load, then strip weight-norm for a clean
                # canonical checkpoint (mirrors the trainer's setup/teardown).
                snap = type(m)(m.config)
                snap.decoder.apply_weight_norm()
                for flow in snap.flow.flows:
                    torch.nn.utils.weight_norm(flow.conv_pre)
                    torch.nn.utils.weight_norm(flow.conv_post)
                snap.load_state_dict(sd, strict=False)
                snap.decoder.remove_weight_norm()
                for flow in snap.flow.flows:
                ...
                snap.save_pretrained(ckpt_dir)
                feature_extractor.save_pretrained(ckpt_dir)
                tokenizer.save_pretrained(ckpt_dir)
                logger.info(f"[snapshot] saved epoch {epoch + 1} -> {ckpt_dir}")
                del snap, sd
        accelerator.wait_for_everyone()
